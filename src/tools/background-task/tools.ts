...
import type { BackgroundManager, BackgroundTask } from "../../shared-hooks/background-agent";
...
export function createBackgroundTask(manager: BackgroundManager): ReturnType<typeof tool> {
  return tool({
...
Only running tasks can be cancelled.`;
        ...
        // Fire-and-forget: abort request without awaiting
        client.session
          .abort({
            path: { id: task.sessionID },
          })
